...
import gtk
import gtk.gdk
import webbrowser # XXX: check dependency
...
class MEMainWindow( ListWindow ):
    ...
    def update ( self):
        ...
        if topFrame != None:
            ...
            if isinstance( aWindow, PathwayEditor ):
    ...
    def __guide_activated(self, *args):
        webbrowser.open_new(
            os.path.join( config.doc_dir, 'HOW-TO.htm' ) )

    def __manual_activated(self, *args):
        webbrowser.open_new(
            os.path.join( config.doc_dir, 'Tutorial.htm' ) )
