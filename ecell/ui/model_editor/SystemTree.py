...
class SystemTree( ViewComponent ):
    def __init__( self, aParentWindow, pointOfAttach ):
        ...
        # set up variables
        self.theSelection = []
        self.theSelectionTypeList = [DM_SYSTEM_TYPE, DM_VARIABLE_TYPE, DM_PROCESS_TYPE, DM_PROPERTY_TYPE ]
    ...
    def update ( self, aSystemFullID ):
        ...
        if aSystemFullID == MS_SYSTEM_ROOT:
            ...
            anIter = self.__getIter( ecell.util.getParentSystemIDStringFromFullIDString( aSystemFullID ) )
    ...
    def add_new ( self ):
        ...
        # change selection
        aPath = ecell.util.convertFullIDStringToSystemPath ( self.theSelection[0] )
        newName = self.theModelEditor.getUniqueEntityName( DM_SYSTEM_TYPE, aPath )
        newID = DM_SYSTEM_TYPE + ':' + aPath + ':' + newName
    ...
    def rename ( self, newName, anIter ):
        ...
        if not ecell.util.validateIDString( newName ):
            utils.showPopupMessage(
                utils.OK_MODE,
                "Only alphanumeric characters and _ are allowed in system ids",
                ME_ERROR )
    ...
    def __buildTree( self, fromIter, fromSysID ):
        ...
        parentSysPath = ecell.util.convertFullIDStringToSystemPath ( fromSysID  )
        subSystemList = self.theParentWindow.theModelEditor.getModel().getEntityList ( SYSTEM,  parentSysPath )
        subSystemList = list( subSystemList )
        # call itself recursively
        subSystemList.sort( lambda x,y:(x>y)-(x<y) )
        for aSubSystem in subSystemList:
            self.__buildTree( anIter, ":".join( [DM_SYSTEM_TYPE, parentSysPath, aSubSystem] ) )
    ...
    def __getIter( self, aSysID ):
        aSysPath = ecell.util.convertFullIDStringToSystemPath( aSysID )
    ...
    def __getSysID( self, anIter ):
        ...
        while True:
            ...
            if parentIter == None:
                return ':'.join( [ DM_SYSTEM_TYPE, aSystemPath, anID ] )
