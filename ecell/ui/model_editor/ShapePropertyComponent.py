...
from ecell.ui.model_editor.viewobjects import *
...
class ShapePropertyComponent(ViewComponent):
    ...
    def __EntityLabel_displayed( self, *args ):
        ...
        self.__ShapeProperty_updated(OB_PROP_FULLID,aShapeLabel)
    ...
    def __EntityWidth_displayed( self, *args ):
        ...
        if self.checkNumeric(aShapeWidth):
            self.__ShapeProperty_updated(OB_PROP_DIMENSION_X,float(aShapeWidth))
        else:
    ...
    def __EntityHeight_displayed( self, *args ):
        ...
        if self.checkNumeric(aShapeHeight):
            self.__ShapeProperty_updated(OB_PROP_DIMENSION_Y,float(aShapeHeight))
        else:
            self.updateShapeProperty()
    ...
    def __ShapeType_displayed( self, *args ):
        ...
        if aShapeType == '':
            return
        self.__ShapeProperty_updated(OB_PROP_SHAPE_TYPE,aShapeType)
    ...
    def __ColorFillSelection_displayed( self,*args):
        ...
        if response==gtk.RESPONSE_OK:
            ...
            aDialog.destroy()
            self.setHexadecimal(aColor,OB_PROP_FILL_COLOR)
            self.__ShapeProperty_updated(OB_PROP_FILL_COLOR,self.theHex)
        else:
            aDialog.destroy()
    ...
    def __ColorOutSelection_displayed( self,*args):
        ...
        if response==gtk.RESPONSE_OK:
            ...
            aDa.modify_bg(gtk.STATE_NORMAL,aColor)
            aDialog.destroy()
            self.setHexadecimal(aColor,OB_PROP_OUTLINE_COLOR)
            self.__ShapeProperty_updated(OB_PROP_OUTLINE_COLOR,self.theHex)
        else:
            aDialog.destroy()
    ...
    def setDisplayedShapeProperty(self ,anObjectDict, aType):
        ...
        if len( self.theObjectDict.values()) > 0:
            ...
            self.theFillColor=self.getColorObject( theObject.getProperty(OB_PROP_FILL_COLOR) )
            self.theOutlineColor=self.getColorObject( theObject.getProperty(OB_PROP_OUTLINE_COLOR) )
        else:
    ...
    def updateShapeProperty(self):
        ...
        if len( self.theObjectDict.keys() ) == 1:
            ...
            if theObject.getProperty( OB_PROP_HAS_FULLID ):
                self.theFullID = theObject.getProperty( OB_PROP_FULLID )
                label = self.theFullID.id
                if theObject.getProperty(OB_PROP_TYPE) == OB_PROP_TYPE_SYSTEM:
            ...
            x = str(theObject.getProperty( OB_PROP_DIMENSION_X ))
            y = str(theObject.getProperty( OB_PROP_DIMENSION_Y ))
    ...
    def populateComboBox(self):
        ...
        if self.theType not in [ "None", "Mixed" ]:
            ...
            currentShape = theObject.getProperty(OB_PROP_SHAPE_TYPE)
    ...
    def modifyObjectProperty(self,aPropertyName, aPropertyValue):
        ...
        if  aPropertyName == OB_PROP_OUTLINE_COLOR  or aPropertyName == OB_PROP_FILL_COLOR or aPropertyName == OB_PROP_SHAPE_TYPE :
            # create commandList
            aCommandList = []
        ...
        elif  aPropertyName == OB_PROP_DIMENSION_Y  :
            ...
            objHeight = theObject.getProperty(OB_PROP_DIMENSION_Y)
        ...
        elif  aPropertyName == OB_PROP_DIMENSION_X :
            if len( self.theObjectDict.values() ) != 1 :
                raise "Cannot change dimensions of more than one object!"
            theObjectID, theObject = self.theObjectDict.items()[0]

            objWidth = theObject.getProperty(OB_PROP_DIMENSION_X)
