...
class AutoLayout:
    ...
    def __collectOtherEntities( self, aList ):
        ...
        newList = []
        aType = ecell.util.getTypeFromFullIDString( aList[0] )
        for anEntity in aList:
            if aType == DM_PROCESS_TYPE:
                varrefList = self.theModel.getEntityProperty( anEntity + ":" + ME_PROCESS_VARREFLIST )
                for varref in varrefList:
                    aVarref = ecell.util.getAbsoluteReference( anEntity, varref[ME_VARREF_FULLID] )
                    variableID = ":".join( [ DM_VARIABLE_TYPE ] + aVarref.split(":")[1:] )
                    if variableID not in newList:
                        newList.append( variableID )
            else:
                processList = self.theModel.getEntityProperty( anEntity + ":" + ME_VARIABLE_PROCESSLIST )
    ...
    def __getSecondarySelection( self, primarySelection ):
        ...
        for aFullID in primarySelection:
            ...
            if aFullID in parentDict.keys():
                continue
            aType = ecell.util.getTypeFromFullIDString( aFullID )
    ...
    def __addSuperSystems( self, anEntityFullID ):
        ...
        parentSystemID = ecell.util.getParentSystemIDStringFromFullIDString( anEntityFullID )
    ...
    def __addSubSystems( self, aSystemFullID ):
        ...
        aDict = {}
        systemPath = ecell.util.convertFullIDStringToSystemPath( aSystemFullID )
        # get processes
        processList = self.theModel.getEntityList(DM_PROCESS_TYPE, systemPath )
        processIDList = map( lambda x : ':'.join([DM_PROCESS_TYPE, systemPath, x]), processList )
        for processID in processIDList:
            aDict[processID] = [None]
        # get variables
        variableList = self.theModel.getEntityList(DM_VARIABLE_TYPE, systemPath )
        if "SIZE" in variableList:
            variableList.remove( "SIZE" )
        variableIDList = map( lambda x : ':'.join([DM_VARIABLE_TYPE, systemPath, x]), variableList )
        for variableID in variableIDList:
            aDict[variableID] = [None]
        #this is a temporary solution until clustering is done!
        return aDict
        # get systems
        systemList = self.theModel.getEntityList(DM_SYSTEM_TYPE, systemPath )
        systemIDList = map( lambda x : ':'.join([DM_SYSTEM_TYPE, systemPath, x]), systemList )
    ...
    def __getVariableReferenceList( self ):
        for aDict in self.allSysID.values():
            for fullID in aDict.keys():
                if fullID.startswith( DM_PROCESS_TYPE ):
                    varrefList = self.theModel.getEntityProperty( fullID+':VariableReferenceList' )
                    for aVarref in varrefList:
                        aName = aVarref[0]
                        aVariableID = ecell.util.getAbsoluteReference(
                            fullID, aVarref[1] )
                        vl = aVariableID.split(':')
                        aVariableID = ':'.join( [DM_VARIABLE_TYPE, vl[1], vl[2]] )
                        aCoef = aVarref[2]
                        varParentDict = self.__getSystemDict(
                            ecell.util.getParentSystemIDStringFromFullIDString( aVariableID ) )
    ...
    def __parseEntities( self, systemID ):
        ...
        for fullID in aDict.keys():
            if fullID.startswith(DM_SYSTEM_TYPE ):
...
#                            text += outvnodeid + "->" + invnodeid + " [ label = '_' ];\n"
    ...
    def __createSystems( self, aFullID, parentObjectID = None ):
        ...
        if aFullID == ME_ROOTID:
            ...
            theObjectID = self.theLayout.getUniqueObjectID( OB_PROP_TYPE_SYSTEM )
            relx, rely = self.__convertAbsToRel( bb[0], bb[1], parentSystem )
            self.theLayout.createObject( theObjectID, OB_PROP_TYPE_SYSTEM, aFullID, relx, rely, parentSystem )
            #resize
            theSystemObject = self.theLayout.getObject( theObjectID )
            deltaright = - theSystemObject.getProperty( OB_PROP_DIMENSION_X ) + width
            deltadown = - theSystemObject.getProperty( OB_PROP_DIMENSION_Y ) + height
            theSystemObject.resize( 0, deltadown, 0, deltaright )
        
        aDict = self.__getSystemDict( aFullID )
        
        for fullID in aDict.keys():
            aType = ecell.util.getTypeFromFullIDString( fullID )
            if aType == DM_SYSTEM_TYPE:
                self.__createSystems( fullID, theObjectID )
            elif aType in [ DM_VARIABLE_TYPE, DM_PROCESS_TYPE ]:
...
#                ewidth = self.attributeMap[entityDotID]["width"][0] *  POINTS_PER_INCH +40
    ...
    def __createConnections( self ):
        for aVarref in self.variableReferenceList:
            ...
            anID = self.theLayout.getUniqueObjectID( OB_PROP_TYPE_CONNECTION )
            ...
            aConnObject.setProperty( OB_PROP_SHAPE_TYPE, SHAPE_TYPE_CURVED_LINE )
