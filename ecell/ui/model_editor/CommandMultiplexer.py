...
class CommandMultiplexer:
    ...
    def processCommand( self, aCmd ):
        ...
        if cmdType == "DeleteEntityList":
            ...
            chgdFullID = ecell.util.convertFullPNStringToFullIDString( chgdFullPN )
            chgdProperty = ecell.util.getPropertyNameFromFullPNString( chgdFullPN )
            if chgdProperty == DMINFO_PROCESS_VARREFLIST:
            ...
            for aProperty in propertyList:
                if aProperty == DMINFO_PROCESS_VARREFLIST:
            ...
            objectType = theObject.getProperty( OB_PROP_TYPE )
            if chgdProperty == OB_PROP_SHAPE_TYPE and objectType != OB_PROP_TYPE_CONNECTION:
                newLabel = theObject.getProperty( OB_PROP_LABEL )
                if theObject.getProperty( OB_PROP_HAS_FULLID ):
                    aType = ecell.util.getTypeFromFullIDString( theObject.getProperty( OB_PROP_FULLID ) )
                else:
                    aType = None
                availSpace = theObject.calcLabelParam(newLabel)

                newDimx, newDimy = self.theModelEditor.theShapePluginManager.getMinDims( theObject.getDMType(), chgdValue, theObject.getGraphUtils(),newLabel )

                if availSpace<newDimx:
                    newLabel=theObject.truncateLabel(newLabel,newDimx,availSpace) 
                newDimx, newDimy = self.theModelEditor.theShapePluginManager.getMinDims( theObject.getProperty( OB_PROP_TYPE ), chgdValue, theObject.getGraphUtils(),newLabel )

                oldDimx=theObject.getProperty(OB_PROP_DIMENSION_X)
                oldDimy=theObject.getProperty(OB_PROP_DIMENSION_Y)
            ...
            # Commands that require Issuing modelcommands
            # ( like renaming connections, changing coefficients )
            if chgdProperty == CO_COEF:
                processObject = theObject.getProperty( CO_PROCESS_ATTACHED )
                processFullID = processObject.getProperty( OB_PROP_FULLID )
                aVarrefName =  theObject.getProperty( CO_NAME )
                aFullPN = ecell.util.convertFullIDStringToFullPNString( processFullID, aVarrefName )
                oldValue = ecell.util.copyValue(
                    self.theModelEditor.getModel().getEntityProperty( aFullPN ) )
                for aVarref in oldValue:
                    if aVarref[ values.VARREF_NAME ] == aVarrefName:
                        aVarref[ values.VARREF_COEF ] = chgdValue
                        break
                ...
                aVarrefName =  theObject.getProperty( CO_NAME )
                aFullPN = ecell.util.convertFullIDStringToFullPNString( processFullID, aVarrefName )
                oldValue = ecell.util.copyValue( self.theModelEditor.getModel().getEntityProperty( aFullPN ) )
                for aVarref in oldValue:
                    if aVarref[ values.VARREF_NAME ] == aVarrefName:
                        aVarref[ values.VARREF_NAME ] = chgdValue
                        break
                chgCmd = ChangeEntityProperty( aFullPN, oldValue )
                returnCmdList.insert(0,chgCmd)
            ...
            if newParent != None:
                ...
                if oldParent != newParent:
                    ...
                    targetSystemID = newParent.getProperty( OB_PROP_FULLID )
            ...
            if isinstance( aBuffer, MultiObjectBuffer ):
            ...
            processFullID = aProcessObject.getProperty( OB_PROP_FULLID )
            process = self.theModelEditor.getModel().getEntity( processFullID )
            aVarrefName = cmdVarrefName
            aVarrefMap = process.variableReferences
            # must be validated beforehand
            assert not aVarrefMap.has_key( aVarrefName )
            variableObject = aCmd.theReceiver.getObject( variableObjectID )
            variableFullID = variableObject.getProperty( OB_PROP_FULLID )
            variable = self.theModelEditor.getModel().getEntity( variableFullID )
            coef = cmdDirection == VARIABLE_TO_PROCESS and -1 or 1
            process.addVariableReference( variable, coef, aVarrefName )
            ...
            oldProcessFullID = oldProcessObject.getProperty( OB_PROP_FULLID )
            ...
            oldVarrefFullPN = ecell.util.convertFullIDStringToFullPNString ( oldProcessFullID, DMINFO_PROCESS_VARREFLIST )
            aVarrefList = ecell.util.copyValue( self.theModelEditor.getModel().getEntityProperty( oldVarrefFullPN ) )
            ...
            if cmdNewVariableObjectID != None:
                ...
                if not doDetachment and not doAttachment:
                    entityCmd = None
                    objCommands = []
                # just detachment,  first change entity property,
                # then detach other layouts
                elif doDetachment and not doAttachment:
                    foundVarref[ values.VARREF_FULLID ] = ":.:___NOTHING"
                    objCommands = []
                        ...
                        for conObjectID in connectionList:
                            ...
                            if conObject.getProperty( CO_NAME ) == aVarrefName:
                                if (conObject.getLayout().theName== anOldLayoutName and conObject.getID() != cmdObjectID) or conObject.getLayout().theName!= anOldLayoutName:
                                    ...
                                    attachmentPoints = ecell.util.copyValue(conObject.getProperty( CO_ENDPOINT2 ) )
                ...
                # just attachment, first change entity varref,
                # then attach other layouts
                elif doAttachment:
                    ...
                    newVariableFullID = aVariableObject.getProperty( OB_PROP_FULLID )
                    foundVarref[ values.VARREF_FULLID ] = ecell.util.getRelativeReference (oldProcessFullID, aVariableObject.getProperty( OB_PROP_FULLID ))
                    ...
                    while True:
                        ...
                        varList = aLayout.getObjectList(OB_PROP_TYPE_VARIABLE)
                        for aVarID in varList:
                            aVarObj =  aLayout.getObject(aVarID)
                            aVarFullID = aVarObj.getProperty(OB_PROP_FULLID)
                            ...
                            for aConID in connectionList:
                                ...
                                if type( currVarIDAttached ) not in (type( [] ), type([] ), type(None)):
                                    ...
                                    currVarFullID = currVarObj.getProperty(OB_PROP_FULLID)
                ...
                foundVarref[ values.VARREF_NAME ] = cmdNewVarrefName
                objCommands = []
                objectIter = self.theLayoutManager.createObjectIterator()
                objectIter.filterByFullID(oldProcessFullID )
                while True:
    ...
    def __deleteReferringVarrefs( self, aFullID ):
        cmdList = []
        if aFullID.typeCode != VARIABLE:
            return cmdList
        aModel = self.theModelEditor.theModel
        aProcessList = aModel.getEntityProperty( ecell.util.convertFullIDStringToFullPNString ( aFullID , MS_VARIABLE_PROCESSLIST ) )

        for aProcess in aProcessList:
            aVarrefFullPN = ecell.util.convertFullIDStringToFullPNString ( aProcess, DMINFO_PROCESS_VARREFLIST )
            aVarrefList = aModel.getEntityProperty( aVarrefFullPN )
            for aVarref in aVarrefList:
                aVariableFullID = ecell.util.createFullIDStringFromVariableReference( aProcess, aVarref )
    ...
    def __pasteOneObjectBuffer( self, aBuffer, aLayout, aParent, returnCmdList ):
        objectType = aBuffer.getProperty ( OB_PROP_TYPE )
        if objectType in [ OB_PROP_TYPE_PROCESS, OB_PROP_TYPE_VARIABLE, OB_PROP_TYPE_SYSTEM]:
            objectFullID = aBuffer.getProperty( OB_PROP_FULLID )
            parentFullID = aParent.getProperty( OB_PROP_FULLID )
            # modify buffer fullid
            # create it from parent 
            parentSysPath = ecell.util.convertFullIDStringToSystemPath( parentFullID )
    ...
    def __changeStepperID( self, aFullID, newStepperID ):
        ...
        while True:
            ...
            if anObject == None:
                break
            chgCmd = SetObjectProperty( anObject.getLayout(), anObject.getID(), OB_PROP_STEPPER_ID, newStepperID)
    ...
    def __changeVariableReferenceList( self, aProcessFullID, newVarrefList ):
        ...
        aFullPN = ecell.util.convertFullIDStringToFullPNString( aProcessFullID, DMINFO_PROCESS_VARREFLIST )
        ...
        for anOldVarref in oldVarrefList:
            ...
            if not foundInNew:
                if changedTo != None:
                    cmdList.extend( self.__changeVarrefName( aProcessFullID, anOldVarref [ values.VARREF_NAME ], changedTo ) )
                else:
                    cmdList.extend( self.__deleteConnections( aProcessFullID, anOldVarref [ values.VARREF_NAME ] ) )

        # cycle through all new varrefs
        for aNewVarref in newVarrefList:
            # process insertions
            if aNewVarref[ values.VARREF_NAME ] not in oldVarrefNames:  
            ...
            if anOldVarref[ ME_VARREF_FULLID ] != aNewVarref[ ME_VARREF_FULLID ]:
                ...
                oldVariable = ecell.util.getAbsoluteReference( aProcessFullID, anOldVarref[ ME_VARREF_FULLID ] )
                newVariable = ecell.util.getAbsoluteReference( aProcessFullID, aNewVarref[ ME_VARREF_FULLID ] )
    ...
    def __changeFullID( self, aSystemObject, newFullID ):
        returnCmdList = []
        newPath = ecell.util.convertFullIDStringToSystemPath( newFullID )
        aLayout = aSystemObject.getLayout()
        for anID in aSystemObject.getObjectList():
            anObject = aLayout.getObject( anID )
            aType = newFullID.typeCode
            if not anObject.getProperty( OB_PROP_HAS_FULLID ):
                continue
            aFullID = anObject.getProperty( OB_PROP_FULLID )
    ...
    def __relabelObject( self, anObject, aType, aFullID ):
            ...
            oldDimx=anObject.getProperty(OB_PROP_DIMENSION_X)
            deltaWidth=newDimx-oldDimx
            #no resize necessary for system child if newDimx is smaller
            if aType == SYSTEM:
            ...
            renameCommand = SetObjectProperty( aLayout, anID, OB_PROP_FULLID, aFullID )
            relabelCommand = SetObjectProperty( aLayout, anID,OB_PROP_LABEL, newLabel)
            returnCmdList.append( resizeCommand )
    ...
    def __checkBrokenConnections(self,oldFullID, newFullID ):
        ...
        oldName = oldFullID.id
        # should find whether there are broken connections when renaming system and copying, cutting entities.
        sysPath = oldFullID.getSuperSystemPath()
        variableObjList = self.theModelEditor.getModel().getEntity( oldFullID ).getChildren( DM_TYPE_VARIABLE )
        variableList = [
            identifiers.FullID( VARIABLE, sysPath, obj.localID )
            for obj in variableObjList ]
        for aVariableFullID in variableList:
            # create the variablefullId after system renamed
            aFullPN = ecell.util.convertFullIDStringToFullPNString ( aVariableFullID, MS_VARIABLE_PROCESSLIST)
            aProcessList = ecell.util.copyValue( self.theModelEditor.getModel().getEntityProperty( aFullPN ) )
            (t, variablePath, variableName) = aVariableFullID.split(':')
            for aProcessFullID in aProcessList:
                aProFullPN = ecell.util.convertFullIDStringToFullPNString ( aProcessFullID, DMINFO_PROCESS_VARREFLIST )
                aVarrefList = ecell.util.copyValue( self.theModelEditor.getModel().getEntityProperty( aProFullPN) )
                for aVarref in aVarrefList:
                    aFullID = aVarref[ME_VARREF_FULLID ]
                    refValid = True
                    if not ecell.util.isAbsoluteVariableReference( aFullID ):
                        try:
                            absFullID = ecell.util.getAbsoluteReference( aProcessFullID, aFullID )
                        except:
                            refValid = False
                    else:
                        absFullID = aFullID
        ...
        systemObjList = self.theModelEditor.getModel().getEntity( oldFullID ).getChildren( DM_TYPE_SYSTEM )
        systemList = [
            identifiers.FullID( SYSTEM, sysPath, obj.localID )
            for obj in systemObjList ]
        if systemList != []:
