...
from ecell.ui.model_editor.PropertyEditor import *
...
class PropertyList( ViewComponent ):
    def __init__( self, aParentWindow, pointOfAttach ):
        ...
        renderer = gtk.CellRendererToggle()
        column=gtk.TreeViewColumn( 'Dynamic', renderer, active = 3)
        column.set_visible( True )
        column.set_resizable( True )
        self['theTreeView'].append_column(column)
        ...
        self.theSelectionTypeList = [ DM_PROPERTY_TYPE ]
        self.theModelEditor = self.theParentWindow.theModelEditor
        self.theModel = self.theModelEditor.getModel()
    ...
    def __drag_data_get( self, *args ):
        ...
        for aName in self.theSelection:
            ...
            fullpn = identifiers.FullPN(
                self.theModel.getFullIDOf(
                    self.theDisplayedEntity ), aName )
    ...
    def getPropertyValue( self, aName ):
        try:
            aSlot = self.theDisplayedEntity.getPropertySlot( aName )
            if aSlot.descriptor.isGettable:
                return aSlot.value
            else:
                return ''
        except:
    ...
    def getPropertyDescriptor( self, aName ):
        return self.theDisplayedEntity.getPropertySlot( aName ).descriptor

    def getPropertyList( self ):
        if self.theDisplayedEntity:
            return self.theDisplayedEntity.getPropertySlots()
        else:
            return []
    ...
    def canAddNewProperty( self ):
        ...
        return self.theDisplayedEntity.klass.acceptsNewProperty
    ...
    def getADCPFlags( self, aType ):
        ...
        if  len( self.theSelection) > 0:
            for aName in self.theSelection:
                if self.getPropertyDescriptor( aName ).isDynamic:
                    flag = True
                    break
    ...
    def getMenuItems( self ):
        ...
        if editFlag :
            editFlag = self.getPropertyDescriptor( self.theSelection[0] ).isSettable
        aMenu.append( [ "edit", editFlag ]  )
        if not isinstance( self.theDisplayedEntity, objs.Stepper ):
            tracerFpnList = []
            for fpn in self.getFullPNList():
                if self.getPropertyDesecriptor( fpn.propertyName ).typeCode in ( DM_PROP_TYPE_INTEGER, DM_PROP_TYPE_FLOAT ):
    ...
    def setDisplayedEntity( self, anEntity ):
        self.theDisplayedEntity = anEntity
        if anEntity == None:
    ...
    def getSelectedIDs( self ):
        ...
        """
        return ecell.util.copyValue( self.theSelection )
    ...
    def selectByUser( self ):        
        ...
        self.changeSelection( ecell.util.copyValue( self.__getSelection() ), True ) 
    ...
    def copy( self ):
        if isinstance( self.theDisplayedEntity, objs.Stepper ):
...
self.theDisplayedEntity, newID, aValue, aType)
            ...
            fpn = identifiers.FullPN(
                self.theModel.getFullIDOf( self.theDisplayedEntity ),
                newID )
...
self.theDisplayedEntity, self.theSelection )
    ...
    def rename ( self, newName, anIter ):
        ...
        if aCommand.isExecutable():
            ...
            utils.showPopupMessage(
                utils.OK_MODE,
                "%s cannot be renamed to %s" % (changedID, newName),
                ME_ERROR )
            self.theListStore.set_value( anIter, 0, changedID )
        ...
        if self.theModelEditor.getMode() == ME_RUN_MODE:
            ...
            if isinstance( self.theDisplayedEntity, objs.Stepper ):
                self.theModel.setEntityProperty(
                    identifiers.FullPN( self.theDisplayedEntity, aName ),
                    newValue )
            else:
                self.theModel.setEntityProperty(
                    identifiers.FullPN( self.theDisplayedEntity, aName ),
    ...
    def edit( self ):
        ...
        if self.isEditable( aName ):
            ...
            aPropertyEditor = PropertyEditor(
                aName,
                self.getPropertyDescriptor( aName ).typeCode,
                self.getPropertyValue( aName ),
                self )
    ...
    def isEditable( self, aName ):
        if aName in [ DMINFO_PROCESS_VARREFLIST, DMINFO_PROCESS_STEPPERID ]:
            return False
    ...
    def __setDeleteButton(self,*args):
        ...
        if len(selected)>0:
            for each in selected:
                canDelete.append(self.getPropertyDynamic(each))
        if len(canDelete)>0:
            for d in canDelete:
                if d==0:
                    self.delButton.set_sensitive(False)
    ...
    def __buildList( self ):
        ...
        for anID in self.getPropertyList():
            ...
            if propValue != None:
                propDesc = self.getPropertyDescriptor( anID )
                aValue.append( propValue )
                aValue.append( propDesc.isSettable )
                aValue.append( propDesc.isDynamic )
                aValue.append( propDesc.isDynamic and self.canAddNewProperty() )
                aValue.append( self.isEditable( anID ) )
                aValue.append( propDesc.isLoadable )
                aValue.append( propDesc.isSaveable )
                aValue.append( propDesc.typeCode )

                if propDesc.isSaveable:
    ...
    def __foreachCallBack( self, *args ):
        ...
        self.thePathList.append( args[1] )
