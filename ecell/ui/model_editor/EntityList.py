...
class EntityList(ViewComponent):
    def __init__( self, aParentWindow, pointOfAttach, anEntityType ):
        ...
        self.theSelectionTypeList = [ self.theType, DM_PROPERTY_TYPE ]
...
#        self['theTreeView'].connect('cursor-changed' , self.__cursor_changed )
    ...
    def changeDisplayedType( self, aType ):
        ...
        if self.theDisplayedSysID != None:
            aNameList = self.theModelEditor.getModel().getEntityList ( self.theType, ecell.util.convertFullIDStringToSystemPath( self.theDisplayedSysID ) )
    ...
    def selectByUser( self ):
        ...
        # get selected sysid  
        aNameList = ecell.util.copyValue( self.__getSelection() )            
        aSelectionList = []
        
        aPath = ecell.util.convertFullIDStringToSystemPath( self.theDisplayedSysID )
    ...
    def paste ( self ):
        ...
        if aBuffer.getType() == DM_PROPERTY_TYPE:
            ...
            pastePath = ecell.util.convertFullIDStringToSystemPath( self.theDisplayedSysID )
    ...
    def add_new ( self ):
        ...
        displayedPath = ecell.util.convertFullIDStringToSystemPath( self.theDisplayedSysID )
        
        if self.theType == DM_PROCESS_TYPE:
            newClass = self.theModelEditor.getDefaultProcessClass()
        elif self.theType == DM_VARIABLE_TYPE:
            newClass = DM_VARIABLE_TYPE
    ...
    def rename ( self, newName, anIter ):
        ...
        if oldName == newName:
            return
        oldTuple = [ self.theType, ecell.util.convertFullIDStringToSystemPath( self.theDisplayedSysID ), oldName ]
        oldID = ':'.join( oldTuple )
    ...
    def __getSelectedRows( self ):
        ...
        #self.thePathList returns index of the items on the entityList        
