...
class MEVariableReferenceEditor(ViewComponent):
    ...
    def getMenuItems( self ):
        ...
        if len( self.theSelection ) == 1:
            ...
            if ecell.util.isAbsoluteVariableReference( aVariable ):
                aMenu.append( [ "turn relative", True ] )
            elif ecell.util.isRelativeVariableReference( aVariable ):
    ...
    def turn_absolute( self ):
        ...
        try:
            aVarref[ME_VARREF_FULLID] = ecell.util.getAbsoluteReference( self.theProcessID, aVarref[ME_VARREF_FULLID] )
        except:
            utils.showPopupMessage(
                utils.OK_MODE,
                "Reference %s cannot be turned into absolute" % \
    ...
    def turn_relative( self ):
        aVarref = self.getVarref(self.theSelection[0])
        try:
            aVarref[ME_VARREF_FULLID] = ecell.util.getRelativeReference( self.theProcessID, aVarref[ME_VARREF_FULLID] )
        except:
            utils.showPopupMessage(
                utils.OK_MODE,
                "Reference %s cannot be turned into relative" % \
                    aVarref[ME_VARREF_FULLID],
                ME_ERROR )
        self.update()
    ...
    def selectByUser( self ):
        # get selected sysid
        aSelectionList = ecell.util.copyValue( self.__getSelection() )

        # call changeselection
        self.changeSelection( aSelectionList, True )

    def getValue( self):
        return ecell.util.copyValue( self.theVarrefList )
    ...
    def __buildList( self ):
        ...
        for aVarref in self.theVarrefList:
            ...
            if not ecell.util.isAbsoluteVariableReference( aFullID ):
                try:
                    aFullID = ecell.util.getAbsoluteReference( self.theProcessID, aFullID )
    ...
    def __nameEdited( self, *args ):
        ...
        if self.getVarref( newName )  == None:
            ...
            utils.showPopupMessage(
                utils.OK_MODE,
                "Cannot rename to %s because it is already used."% newName,
                ME_ERROR )
