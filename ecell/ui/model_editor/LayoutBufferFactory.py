...
from ecell.ui.model_editor.LayoutBuffer import *
...
class LayoutBufferFactory( object ):
    ...
    def createLayoutBuffer( self, aLayoutName ):
        ...
        systemList = aLayout.getObjectList( OB_PROP_TYPE_SYSTEM )

        # add them to systemlist buffer
        for systemObjectID in systemList:
            systemObject = aLayout.getObject( systemObjectID )
            if systemObject.getParent() != aLayout or systemObject.getProperty( OB_PROP_TYPE ) != DM_SYSTEM_TYPE:
                continue
    ...
    def createMultiObjectBuffer( self, aLayoutName, anObjectIDList ):
        ...
        for anObjectID in anObjectIDList:
            ...
            left = anObject.getProperty( OB_PROP_POS_X )
            top = anObject.getProperty( OB_PROP_POS_Y )
            right = left + anObject.getProperty( OB_PROP_DIMENSION_X )
            bottom = top + anObject.getProperty( OB_PROP_DIMENSION_Y )
        ...
        aMultiBuffer.getPropertyBuffer().createProperty( OB_PROP_POS_X , topleftX )
        aMultiBuffer.getPropertyBuffer().createProperty( OB_PROP_POS_Y , topleftY )
        aMultiBuffer.getPropertyBuffer().createProperty( OB_PROP_DIMENSION_X ,   bottomRightX - topleftX )
        aMultiBuffer.getPropertyBuffer().createProperty( OB_PROP_DIMENSION_Y ,  bottomRightY - topleftY )
        return aMultiBuffer
    ...
    def __createSystemObjectBuffer ( self, aSystemObject ):
        ...
        for aChildID in childIDList:
            ...
            aType = childObject.getProperty( OB_PROP_TYPE )
            if aType == OB_PROP_TYPE_SYSTEM:
            ...
            elif aType != OB_PROP_TYPE_CONNECTION:
...
class LayoutBufferPaster:
    ...
    def pasteMultiObjectBuffer( self, aLayout, aBuffer, x, y, theParentID ):
        ...
        offsetX = aBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_X ) - x
        offsetY = aBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_Y ) - y
        translationList = {}
        pastedList = []
        for aSystemBufferName in aBuffer.getSystemObjectListBuffer().getObjectBufferList():
            ...
            posX = aSystemBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_X ) - offsetX
            posY = aSystemBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_Y ) - offsetY
        ...
        for aBufferName in aBuffer.getObjectListBuffer().getObjectBufferList():
            ...
            posX = anObjectBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_X ) - offsetX
            posY = anObjectBuffer.getPropertyBuffer().getProperty( OB_PROP_POS_Y ) - offsetY
    ...
    def pasteObjectBuffer( self, aLayout, aBuffer, x = None, y = None, theParentID = None, translationList = None, pastedList = None):
        ...
        if aType in ( OB_PROP_TYPE_VARIABLE, OB_PROP_TYPE_PROCESS, OB_PROP_TYPE_TEXT, OB_PROP_TYPE_SYSTEM):
            if theParentID != None:
                theParent = aLayout.getObject( theParentID )
            else:
                theParent = None
    ...
    def __pasteObjectBuffer( self, aLayout, aBuffer, x, y, theParent, translationList ):
        ...
        if x == None:
            x = aBuffer.getProperty( OB_PROP_POS_X )
        if y == None:
            y = aBuffer.getProperty( OB_PROP_POS_Y )
        aFullID = None
        if aBuffer.getProperty( OB_PROP_HAS_FULLID ):
            anObjName = aBuffer.getProperty( OB_PROP_FULLID ).id
            if theParent !=None:
                aSystemPath = theParent.getProperty( OB_PROP_FULLID ).toSystemPath()
            else:
                aSystemPath = None
            aFullID = identifiers.FullID( aType, aSystemPath, anObjName )
        ...
        for aProperty in propertyList:
            if aProperty not in (OB_PROP_POS_X , OB_PROP_POS_Y, OB_PROP_FULLID):
                aValue = aBuffer.getProperty( aProperty )
                anObject.setProperty( aProperty, aValue )
            anObject.setProperty(OB_PROP_POS_X, x )
            anObject.setProperty(OB_PROP_POS_Y, y )
            anObject.setProperty( OB_PROP_FULLID, aFullID )
    ...
    def __pasteObjectConnections( self, aLayout, aBuffer, theParent, translationList, pastedList):
        ...
        elif aType in [ OB_PROP_TYPE_VARIABLE, OB_PROP_TYPE_PROCESS]:
    ...
    def __pasteConnectionObjectBuffer( self, aLayout, conBuffer, translationList ):
        ...
        if not conBuffer.getUndoFlag():
            newID = aLayout.getUniqueObjectID( OB_PROP_TYPE_CONNECTION )
        else:
            newID = anID
